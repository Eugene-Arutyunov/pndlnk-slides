...
На каждом слайде есть навигационная колонка, класс dkcp-base-nav-column.

В эток колонке предусмотрен заголовок h1. Этот заголовок нужно взять из слайда — но только если он явно отмечен как заголовок первого уровня (# в маркдауне). В этом случае заголовок переносится из тела слайда в колонку (и в слайде не дублируется)
...
Если в исходнике написано «взять из модели», это значит, что нужная формулировка есть в файле dkcp-template.html — нужно выбрать её, понимая контекст слайда. Например, если слайд про клиентский сегмент, нужно использовать переменную segment. Значение копировать не надо, надо обратиться к переменной. Образец использования переменных есть в файле includes/dkcp.html

Чтобы правильно понять, как что называется, нужно изучить файл DKCP.md

Если на слайде написано «здесь инклюд, вот образец», надо вставить инклюд, как показано в образце
